...
### Added
- **Merchant autocomplete.** The spending dialog's merchant field now suggests merchants you've entered before as you type, showing the category you usually give each — and picking one fills that category automatically. Learned from your own history, ranked by prefix match then frequency.
- **Entries search, category chart, and CSV export.** The Entries ledger gains a **live search** (filters by category, comment, or date as you type) and a **category filter**, plus an **Export CSV** button for the current view. The overview's monthly chart gains a **Total / By-category toggle** — the by-category view stacks each month by its top categories (the rest folded into "Other"), with a legend and per-segment tooltip.
- **Monthly-spending bar chart on the overview.** A 12-month trend chart tops the Spending overview — one bar per calendar month, the current/selected months in solid accent and the rest dimmed; click a bar to jump to that month. Theme-aware and privacy-aware, with a per-bar tooltip.
- **Spending Detail page + overview period selector, and a nicer import.** The Overview has a period selector (This month · 3M · 6M · YTD · 1Y · All), where the multi-month options span **whole calendar months** and month mode keeps month-by-month stepping. A new **Detail** page shows a per-category **month-over-month / year-over-year** table with the **period-over-period change (amount + %)** under each cell (an increase in spending is red, a decrease green), limited to the six most recent periods as a contiguous run — a gap month in the middle stays as a $0 column (so the period-over-period comparison is honest), and only leading empty periods are trimmed (two years of history shows two/three columns, not six). In the importer's category step, choosing **"Create new"** now lets you **type the category's name** instead of always taking the bank's label.
- **Import spendings from a bank export.** A wizard at `/spending/import` reads a PDF export, auto-detects the format, and walks you through mapping bank **accounts → owners** and bank **categories → your categories** (creating new ones on the fly), remembering every choice for next time. Only real expenses are imported: income and **internal movements — account transfers and credit-card payments — are excluded so paying off a card doesn't double-count the purchases the export already lists**. Ambiguous rows (bank fees, cash withdrawals, uncategorized) are surfaced in a dedicated **"Uncertain" step** for an explicit include/skip decision (remembered per category). Re-importing the same file is idempotent (identical rows de-duplicated via deterministic ids). The importer is a **plugin registry**: each format is a small `SpendingImporter` (`detect` + `parse`) added in one place, so supporting another bank/CSV is additive. The first plugin is **Banque Nationale**, parsed by column geometry (robust to pdf.js fragmenting text and to wrapped descriptions). `pdfjs` is dynamically imported so it (and its worker) code-split out of the main bundle — the app only pays for it on the import screen
