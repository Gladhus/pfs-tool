...
### Added
- **Spending Detail page + overview period selector, and a nicer import.** The Overview now has a period selector (This month · 3M · 6M · YTD · 1Y · All, with month-by-month stepping in month mode). A new **Detail** page shows a per-category **month-over-month / year-over-year** table, limited to the six most recent periods that actually have data (empty periods are skipped — two years of history shows two columns, not six). In the importer's category step, choosing **"Create new"** now lets you **type the category's name** instead of always taking the bank's label.
