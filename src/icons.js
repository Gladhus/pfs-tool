// Lucide-style 24x24 stroke icons. Inline SVG strings keep bundle small.
...
// Map category id → icon name. Falls back to 'other' for unknown ids.
...
// Returns 'investments' | 'real-estate' | 'cash' | 'debts' | 'other' — used as
// a CSS modifier so we can target `.cat-investments`, `.cat-debts` etc.
export function categoryKey(categoryId) {
  if (!categoryId) return 'other';
  const id = String(categoryId).toLowerCase().replace(/[\s-]/g, '_');
