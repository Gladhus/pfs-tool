...
import { LEGACY_SELF_ID, viewerShare } from './ownership';
 ...
 * `viewer` is a person id, or HOUSEHOLD_VIEWER for everyone combined; defaults to the legacy self-only view.
 */
export function signedMain(account: Account, balanceRaw: number, main: Currency, usdCad: number | null, viewer: string = LEGACY_SELF_ID): number {
  const converted = toMain(balanceRaw, account.currency ?? main, main, usdCad);
  return converted * viewerShare(account.ownership, viewer) * (account.kind === 'debt' ? -1 : 1);
}
